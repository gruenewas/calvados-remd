...
class Protein(Component):
    ...
    def __init__(self, name: str, comp_dict: dict, defaults: dict,dimer:bool,seam_id:int):
    ...
    def bond_check(self, i: int, j: int):
        ...
        if self.dimer:
            seam_i = self.seam_id - 1  
            if (i == seam_i and j == seam_i + 1):
                return False
...
class Dimer(Protein):
    """Dimer-Protein class requiring a single chain PDB. Includes seam and skips bond at seam between monomers"""

    def __init__(self, name: str, comp_dict: dict, defaults: dict, seam_resid = None):
        super().__init__(name, comp_dict, defaults)
        # 1-based residue index where protomer A ends and protomer B starts.
        # If not provided, assume exact 2× halves.
        if seam_resid is None:
            seam_resid = self.nbeads//2
        self.seam_i = int(seam_resid) - 1
    
    def bond_check(self, i: int, j: int):
        """ Define bonded term conditions. """
        
        if (i == self.seam_i and j == self.seam_i + 1):
            return False
