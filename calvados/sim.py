...
class Sim:
    ...
    def make_components(self):
        ...
        for name, properties in self.comp_dict.items():
            molecule_type = properties.get('molecule_type', "protein")
            if molecule_type == 'protein':
                # Protein component
                comp_setup = 'compact'
                seam = properties.get("seam_resid")
                is_dimer = properties.get("is_dimer")
                comp = Protein(name, properties, self.comp_defaults,is_dimer,seam)    
    ...
    def count_components(self):
        ...
        self.nproteins = np.sum([c.nmol if c.molecule_type == 'protein' else 0 for c in self.components]) + np.sum([c.nmol if c.molecule_type == 'dimer' else 0 for c in self.components])
        ...
        #print(f"Number of proteins: {self.nproteins}")
        ...
        if self.nmolecules > self.nproteins:
            protein_components = self.components[np.where(molecule_types=='protein' or molecule_types == 'dimer')]
    ...
    def build_system(self):
        ...
        * set restraints
        ...
        if self.topol == 'slab': # proteins + rna
            print(f"Box: \n {self.box[0],self.box[1],self.slab_width}")
        ...
        if self.placement == 'default':
            ...
            fplacement = f"{self.path}/placement.txt"
            if os.path.isfile(fplacement):
                if self.verbose:
                    print(f"Found placement file {fplacement}. Using this order to place components")
                idx = {comp.name : 0 for comp in self.components}
                comp_order = parse_global_chain_names(fplacement)
                for comp_name in comp_order:
                    for cidx,comp in enumerate(self.components):
                        if comp_name == comp.name:
                            if self.verbose:
                                print(f'Component {cidx}, Molecule {idx[comp.name]}: {comp.name}')
                            # particle definitions
                            self.add_mdtraj_topol(comp)
                            self.add_particles_system(comp.mws)
                ...
                if self.verbose:
                    print(f"No placement file was found. Placing components randomly and writing placement to {fplacement}.")
                nmol_placed = {comps.name: 0 for comps in self.components}
                while any(nmol_placed[comps.name] < comps.nmol for comps in self.components):
                    comp = np.random.choice(self.components)
                    if nmol_placed[comp.name] < comp.nmol:
                        # Place the molecule in the system
                        if self.verbose:
                            print(f'Component {list(nmol_placed.keys()).index(comp.name)}, Molecule {nmol_placed[comp.name]}: {comp.name}')
                        open(fplacement, "a", encoding="utf-8").write(f'Component {list(nmol_placed.keys()).index(comp.name)}, Molecule {nmol_placed[comp.name]}: {comp.name}\n')
                        # particle definitions
                        self.add_mdtraj_topol(comp)
                        self.add_particles_system(comp.mws)
    ...
    def simulate(self):
        ...
        if (os.path.isfile(fcheck_in)) and (self.restart == 'checkpoint'):
            if not os.path.isfile(f'{self.path}/{self.sysname:s}.dcd'):
                print(f'Did not find {self.path}/{self.sysname:s}.dcd trajectory to append to!')
            ...
            try:
                if self.minimize_energy:
                    ...
                    simulation.minimizeEnergy()
                     ...
                     print("Energy not minimized!")
            except:
                print(f'Minimizing energy.')
                simulation.minimizeEnergy()
        if self.slab_eq:
            if not os.path.isfile(f'{self.path}/equilibration_final.pdb'):
                ...
                print("Skipping equilbration and using existing equilibration_final.pdb as starting configuration")
        ...
        if self.runtime > 0: # in hours
            ...
            for i in tqdm(range(nbatches),mininterval=1):
                ...
                state_checkpoint = simulation.context.getState(getPositions=True,enforcePeriodicBox=True)
                rep = app.pdbreporter.PDBReporter(f'{self.path}/checkpoint.pdb',0)
                rep.report(simulation,state_checkpoint)
        simulation.saveCheckpoint(fcheck_out)
        ...
        state_final = simulation.context.getState(getPositions=True,enforcePeriodicBox=True)
        rep = app.pdbreporter.PDBReporter(f'{self.path}/{self.sysname}.pdb',0)
        rep.report(simulation,state_final)
        rep = app.pdbreporter.PDBReporter(f'{self.path}/checkpoint.pdb',0)
        rep.report(simulation,state_final)
        ...
        with open(f'{self.path}/{self.sysname}.xml', 'w') as output:
            output.write(openmm.XmlSerializer.serialize(self.system))
        print("Overwritten existing xml file")
...
def run(path='.',fconfig='config.yaml',fcomponents='components.yaml'):
    ...
    if os.path.isfile(f'{mysim.path}/{mysim.sysname}.xml'):
        system_xml = Path(f'{mysim.path}/{mysim.sysname}.xml')
        print(f"Found existing system xml file. Using {system_xml} to build system.")
        try:
            mysim.system = openmm.XmlSerializer.deserialize(system_xml.read_text())
            mysim.pdb_cg = f'{mysim.path}/top.pdb'
        except ValueError:
            print("System xml file seems broken, rebuilding system from scratch")
            mysim.build_system()
    else:
        print(f"No system xml file found. Building system from scratch.")
        mysim.build_system()
    mysim.simulate()
    return mysim


def parse_global_chain_names(log_path):
    """List of component names in the order chains were added (global chain ids)."""
    pat = re.compile(r"Component\s+\d+,\s+Molecule\s+\d+:\s+(.+)$")
    names = []
