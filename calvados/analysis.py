...
class SlabAnalysis:
    def __init__(self,
            ...
            input_pdb = 'equilibration_final.pdb', input_dcd = None,
            centered_dcd = 'traj.dcd',
            ref_chains = None, ref_name = None,
            client_chain_list = [], client_names = [],
            verbose = False, temp = None
            ):
        self.name = name
        self.temp = f"{temp}K" if temp else ""
    ...
    def calc_raw_density_profile(self,start = 10, stop = None, step = 1):
        
        u = mda.Universe(f'{self.input_path}/{self.input_pdb}',
                 f'{self.input_path}/{self.input_dcd}', in_memory=True)
        ...
        avg_profile = np.mean(h_all, axis=0)

        plt.plot(z, avg_profile)
        plt.xlabel("z (Å)")
        plt.ylabel("Density (arb. units)")
        plt.title(f"Raw Density Profile for {self.name}")
        plt.show()

    def center(self, start=None, end=None, step=1,
            center_target = 'ref',method = "hist"):
        """
        ...
        if method == "hist": 
            ...
            with mda.Writer(f'{self.output_path}/{self.centered_dcd}', n_atoms) as W:
                for ts in u.trajectory[start:end:step]:
                    u.atoms.translate(-u.atoms.center_of_geometry() + 0.5 * u.dimensions[:3])
                    W.write(u.atoms)
            e = True
        ...
        if self.verbose:
            ...
            if e:
                print(f"Used geometric centering as no dense phase was found")
    ...
    def calc_concentrations(self,
        ...
        while np.nan_to_num(results["c_dense"]) == 0 and self.pden >= 1. :
            if self.verbose:
                print(f"c_dense could not be determined with pden = {self.pden}.")
                print(f"Reducing pden to {(self.pden - 0.1):.2f} and retrying.")
            self.pden -= 0.1
            results = self.calc_single_conc(h,ref=True)
    ...
    def calc_single_conc(self, h, ref=True):
        ...
        try:
            ...
            print("Error Caluclation failed")
            eden,edil = np.NaN,np.NaN
    ...
    def plot_density_profiles(self,only_ref = False):
        ...
        ax.set(yscale='log')
        ax.set(title=f"{self.name}_{self.temp}")
        ax.legend(fontsize=8)

        fig.tight_layout()
        fig.savefig(f'{self.output_path}/{self.name}_{self.temp}_profiles.pdf')
...
def calc_com_traj(path,sysname,output_path,residues_file,chainid_dict={},start=None,end=None,step=1,input_pdb='equilibration_final.pdb'):
    """
    ...
    for chain_name, chainids in chainid_dict.items():
        ...
        if isinstance(chainids, int):
            chainids = np.array([chainids], dtype=int)
        elif isinstance(chainids, tuple):
            chainids = np.arange(chainids[0], chainids[1]+1, dtype=int)
        else:  
            chainids = np.array(chainids, dtype=int)
        ...
        mws[0] += 2
        mws[-1] += 16
        chain_prop[chain_name]['ids'] = chainids
...
def calc_contact_map(path,sysname,output_path,chainid_dict={},is_slab=False,input_pdb='equilibration_final.pdb'):
    """
